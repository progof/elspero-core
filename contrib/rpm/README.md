...
The RPM spec file provided here is for Elspero-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://elspero.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Elspero git repository](https://github.com/elspero/elspero) but are not part of
...
Currently only `Source10` is used, for the example `elspero.conf` file.
...
in the elspero git repository. At present only an SVG version of the Elspero
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the elspero
...
needed when 0.12.1 is released, a proper fix is already in the Elspero git
master branch.
...
idea to build Elspero Core against a static version of that library compiled
...
The `elspero.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `elspero-server` package. At this point, most RPM based
...
The `elspero-server` package should have SELinux support. How to properly do
...
Elspero-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/elspero/). He has been
packaging Elspero for Fedora at least since 2012.
...
4. I renamed the `elspero` package that contains the Qt GUI to `elspero-core` as
that appears to be how the general population refers to it, in contrast to
`elspero-xt` or `elspero-classic`. I wanted to make sure the general population
