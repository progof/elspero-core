...
###   This script attempts to download the signature file SHA256SUMS.asc from elspero.org
...
WORKINGDIR="/tmp/elspero"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://elspero.org/bin/"
VERSIONPREFIX="elspero-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'elspero-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Elspero Core binary release signing key installed?"
   fi
