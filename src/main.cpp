...
// Copyright (c) 2017-2018 The Elspero Core developers
...
const string strMessageMagic = "Elspero Signed Message:\n";
        ...
        if (fAddressIndex)
        {
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                if (out.scriptPubKey.IsPayToScriptHash()) {
                    ...
                    CElsperoAddress address(destination);
    ...
    for (unsigned int i = 0; i < block.vtx[0].vout.size(); i++) {
        ...
        if(nAccValue > nPOWBlockReward && block.vtx[0].vout[i].nValue > 0) {
            ...
            if(metadata[nPaymentRequestsCount].isStr())
            {
                ...
                if(block.vtx[0].vout[i].nValue != prequest->nAmount || prequest->fState != CFund::ACCEPTED || proposal.Address != CElsperoAddress(address).ToString())
...
//
// Commented - Elspero uses now version control
//
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (pfrom->nVersion != 0)
    {
        ...
        {
            reason = "You are using an old version of Elspero, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70017 && IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Segregated Witness has been enabled and you are using an old version of Elspero, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70020 && IsCommunityFundEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Community Fund has been enabled and you are using an old version of Elspero, please update.";
            fObsolete = true;
        }
