...
const char * const ESCCOIN_CONF_FILENAME = "elspero.conf";
const char * const ESCCOIN_PID_FILENAME = "elspero.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Elspero
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Elspero
    // Mac: ~/Library/Application Support/Elspero
    // Unix: ~/.elspero
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Elspero4";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Elspero4";
#else
    // Unix
    return pathRet / ".elspero4";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", ESCCOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No elspero.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override elspero.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", ESCCOIN_PID_FILENAME));
