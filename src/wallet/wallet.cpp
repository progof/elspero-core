...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CMutableTransaction& txNew, CKey& key)
{
    ...
    if(hasPayment){
        ...
        CElsperoAddress address2(address1);
    ...
    if (GetArg("-stakingaddress", "") != "" && !txNew.vout[txNew.vout.size()-1].scriptPubKey.IsColdStaking()) {
        ...
        if (stakingAddress.read(GetArg("-stakingaddress", "")))
        {
            try {
                ...
                {
                    address = CElsperoAddress(find_value(addressMap, "all").get_str());
                }
                // Or use specified address if present
                if(find_value(addressMap, CElsperoAddress(key.GetPubKey().GetID()).ToString()).isStr())
                {
                    address = CElsperoAddress(find_value(addressMap, CElsperoAddress(key.GetPubKey().GetID()).ToString()).get_str());
                }
        ...
        {
            address = CElsperoAddress(GetArg("-stakingaddress", ""));
        }
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CElsperoAddress(CScriptID(redeemScript)).ToString();
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-elspero-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CElsperoAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CElsperoAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CElsperoAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CElsperoAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CElsperoAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CElsperoAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CElsperoAddress(dest).ToString(), key);
}
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-stakingaddress", strprintf(_("Specify a customised elspero address to accumulate the staking rewards.")));
