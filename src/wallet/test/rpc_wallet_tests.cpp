...
BOOST_AUTO_TEST_CASE(rpc_wallet)
{
    ...
    UniValue r;
    CPubKey demoPubkey;
    CElsperoAddress demoAddress;
    UniValue retValue;
    string strAccount = "walletDemoAccount";
    CElsperoAddress setaccountDemoAddress;
    {
        LOCK(pwalletMain->cs_wallet);

        demoPubkey = pwalletMain->GenerateNewKey();
        demoAddress = CElsperoAddress(CTxDestination(demoPubkey.GetID()));
        ...
        setaccountDemoAddress = CElsperoAddress(CTxDestination(setaccountDemoPubkey.GetID()));
    }
    /*********************************
     * 			setaccount
     *********************************/
    ...
    BOOST_CHECK(CElsperoAddress(retValue.get_str()).Get() == demoAddress.Get());
    ...
    BOOST_CHECK(CElsperoAddress(arr[0].get_str()).Get() == demoAddress.Get());
