...
bool GetAddressesPage::validatePage()
{
    ...
    CElsperoAddress stakingAddress(stakingAddressStr.toStdString());
    CElsperoAddress spendingAddress(spendingAddressStr.toStdString());
...
void ColdStakingAddressPage::initializePage()
{
    ...
    coldStakingAddress = QString::fromStdString(CElsperoAddress(stakingKeyID, spendingKeyID).ToString());

#ifdef USE_QRCODE
    QString uri = "elspero:" + coldStakingAddress;
    image->setText("");
    if(!uri.isEmpty())
    {
        // limit URI length
