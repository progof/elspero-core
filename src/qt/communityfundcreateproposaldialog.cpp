...
{
    ui->setupUi(this);
    GUIUtil::setupAddressWidget(ui->lineEditElsperoAddress, this);
...
{
    bool isValid = true;
    if(!ui->lineEditElsperoAddress->isValid() || (ui->lineEditElsperoAddress->text() == QString("")))
    {
        // Styling must be done manually as an empty field returns valid (true)
        ui->lineEditElsperoAddress->setStyleSheet(STYLE_INVALID);
        ui->lineEditElsperoAddress->setValid(false);
        isValid = false;
    }
...
void CommunityFundCreateProposalDialog::click_pushButtonCreateProposal()
{
    ...
    if(this->validate())
    {
        ...
        // Address
        string Address = ui->lineEditElsperoAddress->text().toStdString().c_str();
        ...
        if (curBalance <= Params().GetConsensus().nProposalMinimalFee) {
            QMessageBox msgBox(this);
            string fee = std::to_string(Params().GetConsensus().nProposalMinimalFee / COIN);
            std::string str = "You require at least " + fee + " ESC mature and available to create a proposal\n";
            msgBox.setText(tr(str.c_str()));
            msgBox.addButton(tr("Ok"), QMessageBox::AcceptRole);
            msgBox.setIcon(QMessageBox::Warning);
            msgBox.setWindowTitle("Insufficient ESC");
            msgBox.exec();
            return;
        }
