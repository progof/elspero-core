...
#include "main.h"
#include "elsperogui.h"
#include "elsperounits.h"
...
const std::string ElsperoGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ElsperoGUI::DEFAULT_WALLET = "~Default";

ElsperoGUI::ElsperoGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    fShowingVoting(0)
{
    ...
    Q_FOREACH(ElsperoUnits::Unit u, ElsperoUnits::availableUnits())
    {
        unitDisplayControl->addItem(QString(ElsperoUnits::name(u)), u);
    }
...
#ifdef ENABLE_WALLET
    if(enableWallet) {
        connect(walletFrame, &WalletFrame::requestedSyncWarningInfo, this, &ElsperoGUI::showModalOverlay);
        connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &ElsperoGUI::showModalOverlay);
        connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &ElsperoGUI::showModalOverlay);
    }
#endif
}

ElsperoGUI::~ElsperoGUI()
{
...
void ElsperoGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/elspero"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Elspero address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and elspero: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Elspero addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Elspero addresses"));
    ...
    openAction->setStatusTip(tr("Open a elspero: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Elspero command-line options").arg(tr(PACKAGE_NAME)));
...
void ElsperoGUI::createToolBars()
{
    if(walletFrame)
    {
...
void ElsperoGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool ElsperoGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ElsperoGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ElsperoGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ElsperoGUI::setWalletActionsEnabled(bool enabled)
{
...
void ElsperoGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void ElsperoGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void ElsperoGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ElsperoGUI::optionsClicked()
{
...
void ElsperoGUI::cfundProposalsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void ElsperoGUI::cfundProposalsOpen(bool fMode)
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void ElsperoGUI::cfundPaymentRequestsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void ElsperoGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void ElsperoGUI::infoClicked()
{
    if(!clientModel)
        return;

    QString link = QString("https://info.elspero.org/");
    QDesktopServices::openUrl(QUrl(link));
}

void ElsperoGUI::showDebugWindow()
{
...
void ElsperoGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void ElsperoGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void ElsperoGUI::openClicked()
{
...
void ElsperoGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void ElsperoGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void ElsperoGUI::gotoCommunityFundPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void ElsperoGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void ElsperoGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void ElsperoGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void ElsperoGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ElsperoGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void ElsperoGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Elspero network", "", count));
    if(walletFrame){
...
void ElsperoGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to Elspero network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to Elspero network..."));
...
void ElsperoGUI::showVotingDialog()
{
...
void ElsperoGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Elspero"); // default title
...
void ElsperoGUI::changeEvent(QEvent *e)
{
...
void ElsperoGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ElsperoGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ElsperoGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(ElsperoUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void ElsperoGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ElsperoGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ElsperoGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ElsperoGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ElsperoGUI::setEncryptionStatus(int status)
{
...
void ElsperoGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ElsperoGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ElsperoGUI::detectShutdown()
{
...
void ElsperoGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ElsperoGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ElsperoGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ElsperoGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ElsperoGUI::subscribeToCoreSignals()
{
...
void ElsperoGUI::unsubscribeFromCoreSignals()
{
...
void ElsperoGUI::updateDisplayUnit(int unit)
{
    // Update the list value
    unitDisplayControl->setCurrentText(ElsperoUnits::name(unit));
}

/** Update the display currency **/
void ElsperoGUI::comboBoxChanged(int index)
{
...
void ElsperoGUI::updatePrice()
{
    ...
    std::thread pThread{[this]{
        try {
            ...
            std::string response;
            std::string url(
                    "https://min-api.cryptocompare.com/data/price?fsym=ESC&tsyms="
                    "BTC,"
                    "EUR,"
                    "USD,"
                    "ARS,"
...
size_t ElsperoGUI::priceUdateWriteCallback(void *contents, size_t size, size_t nmemb, void *userp)
{
    ((std::string*) userp)->append((char*) contents, size * nmemb);
    return size * nmemb;
}

void ElsperoGUI::updateStakingStatus()
{
    updateWeight();
        ...
        else if (nLastCoinStakeSearchInterval && nWeight)
        {
            ...
            if ((fFoundPaymentRequest || fFoundProposal) && !this->fDontShowAgain && (this->lastDialogShown + (60*60*24)) < GetTimeNow()) {
                ...
                if (msgbox.clickedButton()==pButtonInfo) {
                    QString link = QString("https://elspero.org/en/community-fund/");
                    QDesktopServices::openUrl(QUrl(link));
                }
            }
