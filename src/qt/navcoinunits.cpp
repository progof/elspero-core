...
ElsperoUnits::ElsperoUnits(QObject *parent):
    QAbstractListModel(parent),
    unitlist(availableUnits())
{
}

QList<ElsperoUnits::Unit> ElsperoUnits::availableUnits()
{
    QList<ElsperoUnits::Unit> unitlist;

    unitlist.append(ESC); // Elspero
...
bool ElsperoUnits::valid(int unit)
{
    switch(unit)
    {
        case ESC:
...
QString ElsperoUnits::name(int unit)
{
    switch(unit)
    {
        case ESC: return QString("ESC");
...
QString ElsperoUnits::description(int unit)
{
    switch(unit)
    {
        case ESC: return QString("Elspero");
...
qint64 ElsperoUnits::factor(int unit)
{
...
int ElsperoUnits::decimals(int unit)
{
    switch(unit)
    {
        case ESC: return 8;
...
QString ElsperoUnits::format(int unit, const CAmount& nIn, bool fPlus, SeparatorStyle separators)
{
...
QString ElsperoUnits::formatWithUnit(int unit, const CAmount& amount, bool plussign, SeparatorStyle separators)
{
    return format(unit, amount, plussign, separators) + QString(" ") + name(unit);
}

QString ElsperoUnits::formatHtmlWithUnit(int unit, const CAmount& amount, bool plussign, SeparatorStyle separators)
{
...
bool ElsperoUnits::parse(int unit, const QString &value, CAmount *val_out)
{
...
QString ElsperoUnits::getAmountColumnTitle(int unit)
{
    QString amountTitle = QObject::tr("Amount");
    if (ElsperoUnits::valid(unit))
    {
        amountTitle += " ("+ElsperoUnits::name(unit) + ")";
    }
    return amountTitle;
}

int ElsperoUnits::rowCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent);
    return unitlist.size();
}

QVariant ElsperoUnits::data(const QModelIndex &index, int role) const
{
    int row = index.row();
