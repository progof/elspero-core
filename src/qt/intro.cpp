...
void FreespaceChecker::check()
{
    ...
    try {
        ...
        if(fs::exists(dataDir))
        {
            ...
            {
                replyStatus = ST_OK;
                replyMessage = tr("Elspero will try to import an old wallet.dat file.");
            } else {
...
void Intro::pickDataDirectory()
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/elspero"));
     ...
     * override -datadir in the elspero.conf file in the default data directory
     * (to be consistent with elsperod behavior)
     */
...
void Intro::makeDefaultConfF()
{
    ...
    {
        char t[] = R"(##
## elspero.conf configuration file. Lines beginning with # are comments.
##
 
# Network-related settings:

# Run on the test network instead of the real elspero network.
#testnet=0
...
#
# JSON-RPC options (for controlling a running Elspero/elsperod process)
#

# server=1 tells Elspero-Qt and elsperod to accept JSON-RPC commands
#server=0
...
# server=1 tells Elspero-Qt to accept JSON-RPC commands.
# it is also read by elsperod to determine if RPC should be enabled 
