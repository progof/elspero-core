    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return ElsperoUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, ElsperoUnits::separatorNever);
            else
                return ElsperoUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+ElsperoUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
