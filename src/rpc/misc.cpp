    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CElsperoAddress(addr).ToString());
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"elsperoaddress\"\n"
            "\nReturn information about the given elspero address.\n"
            "\nArguments:\n"
            "1. \"elsperoaddress\"     (string, required) The elspero address to validate. This may also be an OpenAlias address.\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"elsperoaddress\", (string) The elspero address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"elsperoaddress\", (string) The elspero staking address part of a cold staking address\n"
            "  \"spendingaddress\" : \"elsperoaddress\", (string) The elspero spending address part of a cold staking address\n"
    ...
    if (isValid)
    {
        ...
        if (address.IsColdStakingAddress(Params())) {
            CElsperoAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CElsperoAddress spendingAddress;
...
#ifdef ENABLE_WALLET
        // Case 1: Elspero address and we have full public key:
        CElsperoAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    ...
    CElsperoAddress address(innerID);
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CElsperoAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"elsperoaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"elsperoaddress\"  (string, required) The elspero address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CElsperoAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CElsperoAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CElsperoAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
