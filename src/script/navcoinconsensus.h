...
#ifndef ESCCOIN_ESCCOINCONSENSUS_H
#define ESCCOIN_ESCCOINCONSENSUS_H

#include <stdint.h>

#if defined(BUILD_ESCCOIN_INTERNAL) && defined(HAVE_CONFIG_H)
#include "config/elspero-config.h"
  ...
  #endif
#elif defined(MSC_VER) && !defined(STATIC_LIBESCCOINCONSENSUS)
...
#define ESCCOINCONSENSUS_API_VER 1
...
typedef enum elsperoconsensus_error_t
{
    ...
    elsperoconsensus_ERR_AMOUNT_REQUIRED,
...
enum
{
    ...
    elsperoconsensus_SCRIPT_FLAGS_VERIFY_CHECKSEQUENCEVERIFY = (1U << 10), // enable CHECKSEQUENCEVERIFY (BIP112)
    elsperoconsensus_SCRIPT_FLAGS_VERIFY_WITNESS             = (1U << 11), // enable WITNESS (BIP141)
};
...
EXPORT_SYMBOL int elsperoconsensus_verify_script_with_amount(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen, int64_t amount,
                                    const unsigned char *txTo        , unsigned int txToLen,
                                    unsigned int nIn, unsigned int flags, elsperoconsensus_error* err);
...
#endif // ESCCOIN_ESCCOINCONSENSUS_H
